package tw.org.iii.rightnow;

/**
 * Created by jr760 on 2017/8/23.
 */

class KeyWordDictionary {
    static final String tip = "tip";
    static final String 台語 = "台語";
    static final String 國語 = "國語";
    static final String 小女孩 = "小女孩";
    static final String 相簿 = "Album";
    static final String Facebook = "Facebook";
    static final String Line = "Line";
    static final String Chrome = "Chrome";
    static final String 天氣 = "Weather";
    static final String GoogleMap = "GoogleMap";
    static final String 繪圖 = "Draw";
    static final String Gmail = "Gmail";
    static final String Skype = "Skype";
    static final String 行事曆 = "Calendar";
    static final String Youtube = "Youtube";
    static final String Ptt = "Ptt";
    static final String 推持 = "Twitter";
    static final String Twitch = "Twitch";
    static final String Messenger = "Messenger";
    static final String 卡提諾 = "卡提諾";
    static final String 威力彩 = "威力彩";
    static final String 大樂透 = "大樂透Check";
    static final String 大福彩 = "大福彩";
    static final String 搜尋Youtube影片 = "我要看";
    static final String 台視 = "台視";
    static final String 中視 = "中視";
    static final String 華視 = "華視";
    static final String 民視 = "民視";
    static final String 三立 = "三立";
    static final String 東森 = "東森";
    static final String 中天 = "中天";
    static final String 公視 = "公視";
    static final String 滾石老歌 = "滾石老歌";
    static final String 打電話 = "打電話給";
    static final String 開Chrome搜尋 = "開Chrome搜尋";
    static final String 在維基百科找資料 = "維基";
    static final String 故事 = "故事";
    static final String 本期發票 = "發票";
    static final String 至對獎發票網頁 = "前往對獎";
    static final String GoogleMapGuide = "怎麼走";
    static final String 附近的 = "附近的";
    static final String Operation = "Operation";
    static final String Operation2 = "Operation2";
    //計算用
    static final String num_0 = "0";
    static final String num_1 = "1";
    static final String num_2 = "2";
    static final String num_3 = "3";
    static final String num_4 = "4";
    static final String num_5 = "5";
    static final String num_6 = "6";
    static final String num_7 = "7";
    static final String num_8 = "8";
    static final String num_9 = "9";
    static final String num_10 = "10";
    static final String num_100 = "100";
    static final String num_1000 = "1000";
    static final String num_10000 = "10000";




}
