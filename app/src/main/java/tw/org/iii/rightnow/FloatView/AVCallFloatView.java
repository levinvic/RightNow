...
import tw.org.iii.rightnow.Live2dTop;
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (Math.abs(xDownInScreen - xInScreen) <= ViewConfiguration.get(getContext()).getScaledTouchSlop()
                    ...
                    Intent intent = new Intent((Activity)getContext(), Live2dTop.class);
                    ((Activity)getContext()).startActivity(intent);

                    Toast.makeText(getContext(), "已幫您導回首頁", Toast.LENGTH_LONG).show();
