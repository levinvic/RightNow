...
import tw.org.iii.rightnow.BOTActivityEvent;
import tw.org.iii.rightnow.Live2D.framework.L2DViewMatrix;
import tw.org.iii.rightnow.Live2D.android.FileManager;
import tw.org.iii.rightnow.Live2D.android.OffscreenImage;
import tw.org.iii.rightnow.Live2D.android.SimpleImage;
        ...
        gl.glPushMatrix();
        {
            ...
            if (bg != null) {
                gl.glPushMatrix();
                {
                    ...
                    float SCALE_Y = 0.1f;
                    gl.glTranslatef(-SCALE_X * accelX, SCALE_Y * accelY, 0);// 搖晃
...
//					, 0xFF000000) ;
    ...
    private void setupBackground(GL10 context) {
        ...
        if (new BOTActivityEvent().NowTime(5, 12)) {
            try {
        ...
        } else if (new BOTActivityEvent().NowTime(12, 17)) {
            try {
                InputStream in = FileManager.open(LAppDefine.BACK_IMAGE_NAME1);
                bg = new SimpleImage(context, in);
                // 描画範囲。画面の最大表示範囲に合わせる
                bg.setDrawRect(
                        LAppDefine.VIEW_LOGICAL_MAX_LEFT,
        ...
        } else if (new BOTActivityEvent().NowTime(17, 24)) {
            try {
                InputStream in = FileManager.open(LAppDefine.BACK_IMAGE_NAME2);
                bg = new SimpleImage(context, in);
                // 描画範囲。画面の最大表示範囲に合わせる
                bg.setDrawRect(
                        LAppDefine.VIEW_LOGICAL_MAX_LEFT,
            ...
            try {
                InputStream in = FileManager.open(LAppDefine.BACK_IMAGE_NAME3);
                bg = new SimpleImage(context, in);
                // 描画範囲。画面の最大表示範囲に合わせる
                bg.setDrawRect(
                        LAppDefine.VIEW_LOGICAL_MAX_LEFT,
